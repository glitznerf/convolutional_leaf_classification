...
def write_log(dropout,epochs,current_timestamp,layers_array,eval_metrics,base_pixels):
    ...
    with open('../backup/settings_doc_2.txt', 'a') as file:
        file.write('Leaf recognition model leaf_cnn_7_b_0_5f_bw fitting of ' + str(current_timestamp) + ':') # Zeitpunkt des Speicherns
...
def create_dataset(data_dir, base_pixels):
    ...
    y = np.array(labels)
    X = np.array(images)
    X = np.expand_dims(X, axis=3)	#Only needed when BW due to missing color dimension
    num_categories = 10
...
def build_architecture(dropout, base_pixels):
    ...
    # Erster Block CL
    model.add(Conv2D(5, 3, activation='relu', padding='same', input_shape=(base_pixels, base_pixels, 1)))
