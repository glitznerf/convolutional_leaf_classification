...
def write_log(dropout,epochs,current_timestamp,layers_array,eval_metrics,base_pixels):
    ...
    with open('../backup/settings_doc_2.txt', 'a') as file:
        file.write('Leaf recognition model leaf_cnn_7_a_1_nguyen128 fitting of ' + str(current_timestamp) + ':') # Zeitpunkt des Speicherns
...
def build_architecture(dropout, base_pixels):
    ...
    # Erster Block CL
    model.add(Conv2D(base_pixels, 3, activation='relu', padding='same', input_shape=(base_pixels, base_pixels, 3)))
    ...
    # Zweiter Block CL
    model.add(Conv2D(base_pixels*2, 3, activation='relu', padding='same'))
    model.add(Conv2D(base_pixels*2, 3, activation='relu', padding='same'))
    model.add(MaxPooling2D(pool_size=(2, 2)))    
    model.add(Dropout(dropout))

    # Dritter Block CL
    model.add(Conv2D(base_pixels*4, 3, activation='relu', padding='same'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(dropout))

    # Dimensionsreduzierung und dichte Schichten
    model.add(Flatten())
    model.add(Dense(base_pixels*4, activation='relu'))
    model.add(Dropout(dropout))
    ...
    layers_array = [0,0,0,1,2,0,0,1,2,0,1,2,3,4,2,4]
