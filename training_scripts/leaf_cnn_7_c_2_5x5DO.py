...
def write_log(dropout,epochs,current_timestamp,layers_array,eval_metrics,base_pixels):
    ...
    with open('../backup/settings_doc_2.txt', 'a') as file:
        file.write('Leaf recognition model leaf_cnn_7_c_2_5x5DO fitting of ' + str(current_timestamp) + ':') # Zeitpunkt des Speicherns
...
def build_architecture(dropout, base_pixels):
    ...
    # Erster Block CL
    model.add(Conv2D(base_pixels, 5, activation='relu', padding='same', input_shape=(base_pixels, base_pixels, 3)))
    model.add(Conv2D(base_pixels, 5, activation='relu', padding='same'))
    model.add(Conv2D(base_pixels, 5, activation='relu', padding='same'))
