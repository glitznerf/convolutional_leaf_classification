...
def write_log(dropout,epochs,current_timestamp,layers_array,eval_metrics,base_pixels):
    ...
    with open('../backup/settings_doc_2.txt', 'a') as file:
        file.write('Leaf recognition model leaf_cnn_7_a_0_vgg fitting of ' + str(current_timestamp) + ':') # Zeitpunkt des Speicherns
...
def build_architecture(dropout, base_pixels):
    ...
    # Erster Block CL
    model.add(Conv2D(64, 3, activation='relu', padding='same', input_shape=(base_pixels, base_pixels, 3)))
    ...
    layers_array = [0,1,0,1,0,0,1,3,4,2,4,2,4,4]
